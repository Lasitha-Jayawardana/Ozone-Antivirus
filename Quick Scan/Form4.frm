...
Dim fsd As Folder
Set fsd = fgso.GetFolder(App.Path)
Dim F As file, t As TextStream
Dim fso As New FileSystemObject
Set F = fso.GetFile(Str)
If fso.FileExists(fsd.ParentFolder & "\Realtime shields\File shield.oz") Then
RichTextBox2.LoadFile fsd.ParentFolder & "\Realtime shields\File shield.oz"
End If
RichTextBox2.Text = vbCrLf & RichTextBox2.Text
RichTextBox2.Find ".", 1
RichTextBox2.Find Str, 1
If RichTextBox2.SelText = "" Or RichTextBox2.SelText = "." Then
Set st = fgso.OpenTextFile(fsd.ParentFolder & "\Realtime shields\File shield.oz", ForAppending, True)
...
Set t = fso.OpenTextFile(fsd.ParentFolder & "\" & "Virus Chest" & "\" & "log" & "\" & "log.oz", ForReading, True)
Dim k As Integer
k = Val(t.ReadLine)
t.Close
Set t = fso.CreateTextFile(fsd.ParentFolder & "\Virus Chest\" & k & " .oz")
t.Write r.Text
t.Close
Set t = fso.CreateTextFile(fsd.ParentFolder & "\Virus Chest\log\log.oz")
t.WriteLine k + 1
t.Close
...
Dim fsd As Folder
Set fsd = fgso.GetFolder(App.Path)
If fgso.FileExists(fsd.ParentFolder & "\Realtime shields\File shield.oz") Then
RichTextBox2.LoadFile fsd.ParentFolder & "\Realtime shields\File shield.oz"
End If
RichTextBox2.Text = vbCrLf & RichTextBox2.Text
RichTextBox2.Find ".", 1
